...
parser.add_argument('--experiment', default='peta', type=str, help='(default=%(default)s)')
...
parser.add_argument('--evaluate', default = False, required=False, help='evaluate model on validation set')
...
def test(val_loader, model, attr_num, description,epoch):
    ...
    with  open("./wptra_peta.txt", "a+") as output_trY:
...
def save_checkpoint(state, epoch, prefix, filename='.pth.tar'):
    ...
    directory = "wp" + args.experiment + '/' + args.approach + '/'
